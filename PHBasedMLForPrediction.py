...
def get_train_model(target_list,rank_list1,split_ratio=0.3,model_pick='dense',epo=3,batc=50,plus=False):  #feature_train,feature_test,target_train,target_test,model,RMSE=get_train_model(['2','3','4','5'],['0'],0.3)
    feature_train=[]
    target_train=[]
...
#                betti_tmp[j3].append(round(far_tmp[j3],2))
#                betti_tmp[j3].append(round(far_tmp[j3]*far_k)/far_k)
            feature_train=numpy.row_stack((feature_train,betti_tmp))
            if label=='energy':
                target_train = numpy.array(target_train)
                energy_tmp = numpy.array(energy_tmp)
                target_train=numpy.row_stack((target_train.reshape(len(target_train),1) ,energy_tmp.reshape(len(energy_tmp),1) ))
    if model_pick=='gbr':
...
def save_feature(feature_train,target_train,feature_test=[],target_test=[],name='feature_target'):
    ...
    for i_a in range(len(all_feature)):
         ...
         if all_target[i_a] >0.5:
             all_feature[i_a] = all_feature[i_a-1]
...
#            feature_comp[i_f]=numpy.array(feature_comp[i_f])
        ...
        plt.figure(dpi=100, figsize=(5, 3))
        plt.scatter(target_comp,pred_comp)
        os.chdir(work_file)
...
def make_pos_fit(pos):
    ...
    pos1=pos2[0]
    atom_distances1 = atom_distances +random.randint(-1,-1)*random.random()*0.15
    maxradius = 0.4 + random.randint(-1,-1)*random.random()*0.1
    minradius = 0.3 + random.randint(-1,-1)*random.random()*0.1
    max_dis0=math.pow(atom_distances1-0.2,2)
    max_dis1=math.pow(atom_distances1+0.4,2)
    max_dis2=math.pow(atom_distances1+0.8,2)
    iter_1=0
        ...
        for i_11 in range(len(pos1)-1):
            ...
            for j_1 in range(i_1+1,len(pos1)):
                ...
                if bef_dis > max_dis0 and bef_dis < max_dis1+0.1 :
                    ltr_dis=math.sqrt(bef_dis/math.pow(atom_distances1,2))
                    ...
                    ltr_dis=math.sqrt(bef_dis/math.pow(atom_distances1+maxradius,2))
                    bef_x=pos1[j_1][0]-pos1[i_1][0]
                    bef_y=pos1[j_1][1]-pos1[i_1][1]
                    bef_z=pos1[j_1][2]-pos1[i_1][2]
                    ltr_x=(bef_x/ltr_dis+bef_x*3)/4
                    ...
                    ltr_dis=math.sqrt(bef_dis/math.pow(atom_distances1-minradius,2))
                    bef_x=pos1[j_1][0]-pos1[i_1][0]
                    bef_y=pos1[j_1][1]-pos1[i_1][1]
                    bef_z=pos1[j_1][2]-pos1[i_1][2]
                    ltr_x=(bef_x/ltr_dis+bef_x*3)/4
                    ...
                    pos1[j_1]=[pos1[i_1][0]+ltr_x,pos1[i_1][1]+ltr_y,pos1[i_1][2]+ltr_z]
                    change_sum=change_sum+abs(ltr_x-bef_x)+abs(ltr_y-bef_y)+abs(ltr_y-bef_y)
        if change_sum<0.005 or iter_1>random.random()*100:
            break                    
    return pos1
...
#                        z_z=random.randint(-1,1)   
                        ...
                        if random.randint(0,1):
                            ...
                            v_tmp=list(map(lambda x,y,z: pso_w*x+y+z,vw[-2][i2][i4],new_pos_c1[i4],new_pos_c2[i4]))
...
def make_dense_model():
    ...
    #model.add(Dropout(0.25))
    #model.add(Flatten())
    for layi in range(dense_layers):
...
atom_distances=3
pso_c2=2
sole_energy=-.29363970E+00
work_file='/udata/chenx/topology/VASP15'
os.chdir(work_file)
far_x=10
#time.sleep(25000)
dense_nodes=800
dense_layers=4
feature_train,feature_test,target_train,target_test,model,RMSE,scaler_mm=get_train_model(['3','4','5','6','8','9','10'],['0','20'],0.3,epo=3,batc=50,plus=True)
#save_feature(feature_train,target_train,feature_test,target_test,name='feature_target_3_10_x_10_j_20')
#node_record=''
#for dense_nodes in range(800,801):
#    for dense_layers in range(5,6,1):
#        for epo in [3]:
#            for batch_i in [50]:
...
#feature_train,feature_test,target_train,target_test,model,RMSE,scaler_mm=load_feature(name='feature_target_3_10_x_10_j_20',split_ratio=0.3,epo=3,batc=50,plus=True)
...
#model_predict_pos('20',100,model,10000,'new6')
...
#for pbs in range(20,21):
#    atomnum=pbs
#    os.chdir('/udata/chenx/topology/VASP15/%d'%atomnum) 
##    ini_pos(atomnum,50,'vasp',True)                                 
#    continue_pos(atomnum,'vasp',True,20,26) 
#    atomnum=pbs
##    
